...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="azunit">
    <test-results name="{@name}" total="{count(//test)}" errors="{count(//test[@result='Failed'])}" failures="0" not-run="{count(//test[@result='Ignored'])}" inconclusive="1" ignored="4" skipped="0" invalid="3" date="{format-dateTime(@startTime, '[Y0001]-[M01]-[D01]')}" time="{format-dateTime(@startTime,'[h01]:[m01]:[s01]')}">
