...
describe("AzuRunResult", function() {
    ...
    describe("#getState()", function() {
        it("should return ignored with empty groups", function() {
            let start = new Date(2000, 1, 1, 0, 0, 0, 0)
            let run = new Results.AzuRunResult("name", "subscription", start);
            assert.equal(run.getState(), Results.AzuState.Ignored);
        });
        it("should return pass with single succeeding test", function() {
            ...
            assert.equal(run.getState(), Results.AzuState.Passed);
        });
        it("should return fail with single failed assertion", function() {
            ...
            group.tests.push(test);
            run.groups.push(group);
            assert.equal(group.getState(), Results.AzuState.Failed);
        });
        it("should return fail with all success assertions and one fail assertion", function() {
            ...
            let assertion3 = new Results.AzuAssertionResult(Results.AzuState.Passed, "message");
            let assertion4 = new Results.AzuAssertionResult(Results.AzuState.Passed, "message");
            let assertion5 = new Results.AzuAssertionResult(Results.AzuState.Passed, "message");
            let assertion6 = new Results.AzuAssertionResult(Results.AzuState.Passed, "message");
            let assertion7 = new Results.AzuAssertionResult(Results.AzuState.Passed, "message");
            let assertion8 = new Results.AzuAssertionResult(Results.AzuState.Passed, "message");
            ...
            assert.equal(run.getState(), Results.AzuState.Failed);
        });
    });
});
...
describe("AzuGroupResult", function() {
    ...
    describe("#getState()", function() {
        it("should return ignored with empty tests", function() {
            ...
            assert.equal(group.getState(), Results.AzuState.Ignored);
        });
        it("should return pass with single succeeding test", function() {
            let start = new Date(2000, 1, 1, 0, 0, 0, 0)
            let group = new Results.AzuGroupResult("name", "source", start);
            ...
            test.assertions.push(assertion);
            group.tests.push(test);
            assert.equal(group.getState(), Results.AzuState.Passed);
        });
        it("should return fail with single failed assertion", function() {
            let start = new Date(2000, 1, 1, 0, 0, 0, 0)
            let group = new Results.AzuGroupResult("name", "source", start);
...
describe("AzuTestResult", function() {
    ...
    describe("#getState()", function() {
        ...
        it("should return ignored with single ignored assertion", function() {
            let test = new Results.AzuTestResult("name");
            let assertion = new Results.AzuAssertionResult(Results.AzuState.Ignored, "message");
            test.assertions.push(assertion);
            assert.equal(test.getState(), Results.AzuState.Ignored);
        });
